...
V = filtered_tender_df  # バイナリ行列（0/1）DataFrame
V_np = V.values         # original（参照用）
...
V = filtered_swollen_df  # バイナリ行列（0/1）DataFrame
V_np = V.values         # original（参照用）
...
df_patient_basis = pd.DataFrame(
    patients_factor,
    index=V.index,       # 患者IDなど
    columns=basis_cols
)

df_joint_basis = pd.DataFrame(
    joints_factor,
    index=V.columns,     # 関節名など
    columns=basis_cols
)
...
ra_joint_basis_sorted = (
    ...
    .assign(
        date=pd.to_datetime(
            ra_joint_basis["date"],
            format="mixed"   # 念のため（混在対策）
        ).dt.date           # ★時刻を落として「日付だけ」にする
    )
    .sort_values(["ID", "date"])
